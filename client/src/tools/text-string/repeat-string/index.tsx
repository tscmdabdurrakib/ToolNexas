...
function RepeatString() {
  ...
  useEffect(() => {
    ...
    const count = Math.min(repeatCount, 1000); // Limit to prevent browser freeze
    setOutput(inputText.repeat(count));
  }, [inputText, repeatCount]);
      ...
      <Card className="border-2 shadow-lg">
        <CardHeader className="text-center space-y-2 pb-4 bg-gradient-to-r from-blue-50 to-cyan-50 dark:from-blue-950/20 dark:to-cyan-950/20">
          <div className="flex items-center justify-center gap-2">
            ...
            <CardTitle className="text-2xl">Repeat a String</CardTitle>
          </div>
          <CardDescription>Repeat your text multiple times instantly</CardDescription>
        </CardHeader>
        <CardContent className="pt-6 space-y-4">
          ...
          <div>
            <Label htmlFor="repeat-count">Repeat Count: {repeatCount}</Label>
            <Input
              id="repeat-count"
              ...
              className="mt-2"
            />
            <p className="text-xs text-muted-foreground mt-1">Maximum 1000 repetitions</p>
          </div>
