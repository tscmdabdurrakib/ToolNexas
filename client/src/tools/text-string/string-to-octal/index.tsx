...
function StringToOctal() {
  ...
  useEffect(() => {
    ...
    try {
      // Convert string to UTF-8 bytes then to octal
      const utf8Bytes = new TextEncoder().encode(inputText);
      const octalArray = Array.from(utf8Bytes).map(byte => 
        byte.toString(8).padStart(3, '0')
      );
      
      setOutput(octalArray.join(' '));
      setByteCount(utf8Bytes.length);
    } catch (error) {
      setOutput('Error converting text to octal');
      setByteCount(0);
    }
  }, [inputText]);
  ...
  const copyToClipboard = async () => {
    ...
    try {
      ...
      toast({
        title: "Copied!",
        description: "Octal representation copied to clipboard",
      });
    } catch (err) {
      toast({
      ...
      <Card className="border-2 shadow-lg">
        <CardHeader className="text-center space-y-2 pb-4 bg-gradient-to-r from-amber-50 to-yellow-50 dark:from-amber-950/20 dark:to-yellow-950/20">
          <div className="flex items-center justify-center gap-2">
            ...
            <CardTitle className="text-2xl">Convert a String to Octal</CardTitle>
          </div>
          <CardDescription>Convert text to octal representation (base-8)</CardDescription>
        </CardHeader>
        <CardContent className="pt-6 space-y-4">
          ...
          <div className="text-center">
            <Card className="p-4 inline-block">
              <div className="text-2xl font-bold text-amber-600 dark:text-amber-400">{byteCount}</div>
              <div className="text-sm text-muted-foreground">Bytes</div>
            </Card>
          </div>

          <div>
            <Label htmlFor="output">Octal Representation:</Label>
            <Textarea
              id="output"
              ...
              placeholder="Octal representation will appear here..."
            />
          </div>
