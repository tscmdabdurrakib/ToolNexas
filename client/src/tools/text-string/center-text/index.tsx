...
function CenterText() {
  ...
  useEffect(() => {
    ...
    const alignedLines = lines.map(line => {
      ...
      const totalPadding = lineWidth - trimmedLine.length;
  ...
  const copyToClipboard = async () => {
    ...
    try {
      ...
      toast({
        title: "Copied!",
        description: "Center-aligned text copied to clipboard",
      });
    } catch (err) {
      toast({
      ...
      <Card className="border-2 shadow-lg">
        <CardHeader className="text-center space-y-2 pb-4 bg-gradient-to-r from-yellow-50 to-amber-50 dark:from-yellow-950/20 dark:to-amber-950/20">
          <div className="flex items-center justify-center gap-2">
            <AlignCenter className="h-6 w-6 text-yellow-600 dark:text-yellow-400" />
            <CardTitle className="text-2xl">Center Text</CardTitle>
          </div>
          <CardDescription>Center-align text within a specified line width</CardDescription>
        </CardHeader>
        <CardContent className="pt-6 space-y-4">
          <div>
            ...
            <Textarea
              ...
              placeholder="Enter text to center-align..."
              value={inputText}
          ...
          <div>
            <Label htmlFor="output">Center-aligned Text:</Label>
            <Textarea
              id="output"
              ...
              placeholder="Center-aligned text will appear here..."
            />
          </div>
